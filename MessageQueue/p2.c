...
void send(char buffer[MAX_BUFFER_SIZE])
{
        ...
        if((recvr_msgq_fd = mq_open(MESS2, O_WRONLY|O_CREAT,0,0)) == -1)
        {
...
void receive()
{
	...
	int recvr_msgq_fd =0;
        ...
        if((recvr_msgq_fd = mq_open(MESS, O_RDONLY|O_CREAT,0660,&attr)) == -1)
        {
                printf("Client Error, errorno:%d",errno);
                exit(1);
        }
	...
	select(recvr_msgq_fd+1,&readfds,NULL,NULL,NULL);
